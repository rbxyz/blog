...
export default function SpotifyPlaylist({ playlistUrl, className = '' }: SpotifyPlaylistProps) {
  ...
  useEffect(() => {
    ...
    // Simular dados da playlist (em produção, você usaria a API do Spotify)
    const mockPlaylistData: SpotifyPlaylistData = {
      name: type === 'track' ? 'Música Relacionada' : 'Playlist Relacionada',
      description: type === 'track' 
        ? 'Música selecionada para acompanhar este conteúdo'
      ...
      tracks: {
        total: type === 'track' ? 1 : 15
      },
      external_urls: {
        spotify: playlistUrl
      },
      type,
      duration_ms: type === 'track' ? 180000 : undefined, // 3 minutos para track
      artists: type === 'track' ? [{ name: 'Artista' }] : undefined,
    };
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1 text-white">
              <h3 className="font-bold text-lg">{playlistData.name}</h3>
              <p className="text-white/80 text-sm">
                {playlistData.type === 'track' && playlistData.artists 
                  ? `${playlistData.artists[0]?.name}`
                  : `${playlistData.tracks.total} ${playlistData.type === 'track' ? 'faixa' : 'faixas'}`
                }
              </p>
            </div>
          </div>
